...
def get_args():
    ...
    parser.add_argument("--A", type=str, required=True, help="Sequence A")
    parser.add_argument("--B", type=str, required=True, help="Sequence B")
    parser.add_argument("--gap", type=int, default=-2, help="Gap penalty (default: -2)")
    parser.add_argument(
        "--miss", type=int, default=-1, help="Mismatch penalty (default: -1)"
    )
...
def sw_fill_matrix(A, B, gap, miss, match):
    ...
    for i in range(1, len(A) + 1):
        for j in range(1, len(B) + 1):
            H[i][j] = max(
                H[i - 1][j - 1] + (match if A[i - 1] == B[j - 1] else miss),
                H[i - 1][j] + gap,
                H[i][j - 1] + gap,
                0,
            )
...
def sw_traceback(H, A, B, gap, miss, match):
    ...
    # If no alignment (all scores are 0), return empty strings and score 0
    if max_score == 0:
        return "", "", 0
...
def sw(A, B, gap, miss, match):
    ...
    # Handle case when alignment is empty
    if not A:
        return 0, "", "", ""
...
def main():
    ...
    score, align_A, align_B, match = sw(args.A, args.B, args.gap, args.miss, args.match)
    print(score)
    print(align_A)
    print(match)
    print(align_B)
